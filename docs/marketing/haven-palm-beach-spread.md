...
- **Live transparency: two of the three sources are gone, and the one left is Jake's call.** Both marks are RGBA PNGs and Chrome writes each as an image plus a soft mask; the old check grepped `/ca` and `/CA` only, so it named the captions as the source and could not see the masks. The marks sit on flat paper, so the build now composites those masks away on the finished PDF and the picture is unchanged: measured against the previous file at the marks' own resolution, the largest difference is **2 levels out of 255**, which is 8-bit rounding. Flattening the source PNGs instead moves ink by up to 199 levels, because Chrome then re-encodes the mark on a different pixel grid and composites it in a different colour space; that version was built, measured and thrown away. **What is left is the three text runs at 70% white** (verso caption, panel photo caption, panel footer), and setting them to their opaque equivalents is a colour edit that needs sign-off - exact and invisible on the navy panel, approximate on the verso caption because it sits over a photograph. X-4 allows it and nothing changes. X-1a forbids it.
