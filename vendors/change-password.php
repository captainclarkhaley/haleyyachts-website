<?php
/**
 * change-password.php - forced first-login / admin-reset password change
 * (config-driven / white-label).
 *
 * Replaces the old static change-password.html. Server-rendered so identity,
 * login copy, logo, favicon, and colors come from suite_settings (branding.php),
 * each falling back to today's OWYG value. The old change-password.html is kept
 * as a thin redirect shim to this file.
 *
 * Gate (mirrors index.php / suite.php, all decided server-side before markup):
 *   - not signed in            -> login.php
 *   - signed in, NOT flagged   -> suite.php (nothing to force; use the app)
 *   - signed in AND flagged    -> show the set-password form
 * The client-side JS keeps its existing 401/409 handling as a backstop.
 */
...
if ((int) $gateUser['must_change_password'] !== 1) {
    // Nothing to force - send them to the app.
...
<head>
    ...
    <title>Set Your Password - <?php echo $h($brandName); ?></title>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .lead { font-size: 0.88rem; color: var(--muted); margin: 0 0 20px; }
        ...
        .link-row button:hover { color: var(--cyan-d); text-decoration: underline; }
    </style>
...
<div class="auth-card">
    <div class="auth-head">
        ...
        <p><?php echo $h($brandName); ?> - set your password</p>
    </div>
    <div class="auth-body">

        <p class="lead">Before you use <?php echo $h($brandName); ?>, please <strong>set your own password</strong>. This replaces the temporary password you were given. This step is required.</p>
        ...
        <form id="pwForm" autocomplete="on">
            <div class="field">
                ...
                <input type="password" id="pw1" name="new-password" autocomplete="new-password" autofocus>
            </div>
            ...
            <button type="submit" class="btn btn-primary" id="btnSet">Set Password &amp; Continue</button>
        </form>
...
(function () {
    ...
    $('pwForm').addEventListener('submit', function (e) {
        ...
        post('force_change_password', { new_password: pw1 }).then(function (d) {
            // Not authenticated (no/expired session): go sign in.
            if (d._status === 401) { window.location.href = 'login.php'; return; }
            $('btnSet').disabled = false;
            if (!d.ok) {
                // 409 means the account is not actually flagged - it can use the app.
                if (d._status === 409) { window.location.href = 'suite.php'; return; }
                showError(d.error || 'Could not set your password.');
                return;
            }
            showOk((d.message || 'Your password has been set.') + ' Taking you to your suite...');
            // Land on the Yacht Broker Support launcher, consistent with the login flow.
    ...
    // The only way out without setting a password is to sign out entirely.
